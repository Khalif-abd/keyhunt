...
std::string urlEncode(const std::string& value) {
    char* output = curl_easy_escape(nullptr, value.c_str(), value.length());
    std::string encodedValue(output);
    curl_free(output);
    return encodedValue;
}
...
void sendMessage(const std::string& token,  const std::string& chat_id, const std::string& text) {
    ...
    std::string url = "https://api.telegram.org/bot" + encodedToken + "/sendMessage?chat_id=" + encodedChatId + "&text=" + encodedText;
    CURL *curl;
    CURLcode res;
